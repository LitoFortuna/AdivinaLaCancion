...
import { getDailyIndex, getDailyKey, updateStats, loadDailyStats, saveDailyStats, CLIP_DURATIONS } from '../utils/gameLogic'
...
export default function DailyGame() {
  ...
  const track  = tracks.length > 0 ? tracks[getDailyIndex(tracks)] : null

  const savedState = (() => {
    try { return JSON.parse(localStorage.getItem(dayKey) || 'null') } catch { return null }
  })()

  const { state, submitGuess, skip } = useGame(track, savedState)
  const { playing, progress, currentRound, play, advanceRound } = useAudio(track?.previewUrl)
  ...
  useEffect(() => {
    ...
    if (saved) {
      try { if (JSON.parse(saved).status !== 'playing') setShowResult(true) } catch {}
    }
  }, [track, dayKey])
  ...
  useEffect(() => {
    ...
    saveDailyStats(updateStats(loadDailyStats(), state))
  ...
  return (
    <>
      ...
      <div className="game-root">
        ...
        {/* LEFT column — album art card (desktop) + mode info + player */}
      ...
      {showResult && (
        <ResultModal state={state} track={track} onClose={() => setShowResult(false)} isDaily />
      )}
    </>
  )
}
